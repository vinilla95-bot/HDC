      ...
      <>
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <div style={{ fontSize: 14, fontWeight: 800, color: "#28a745", marginBottom: 12 }}>
              ✅ 작업지시완료
            </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 14, fontWeight: 800, color: "#ffc107", marginBottom: 12 }}>
              🚚 출고대기 <span style={{ background: "#ffc107", color: "#000", padding: "2px 8px", borderRadius: 10, fontSize: 12 }}>{waitingItems.length}대</span>
            </div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              {["3x3", "3x4", "3x6", "3x9"].map(spec => (
        ...
        <div style={{ background: "#fff", borderRadius: "0 0 12px 12px", border: "1px solid #e5e7eb", borderTop: "none", overflow: "hidden" }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: 40, color: "#888" }}>재고 데이터가 없습니다.</div>
          ) : (
              ...
              <table className="inventory-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tbody>
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item.quote_id} style={{ background: isCompleted ? "#f0f0f0" : "#fff", opacity: isCompleted ? 0.6 : 1, borderBottom: "1px solid #eee", outline: isUnpaid && !isCompleted ? "2px solid #dc3545" : "none" }}>
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                          <select value={item.inventory_status || "작업지시완료"} onChange={(e) => updateField(item.quote_id, "inventory_status", e.target.value)} style={{ padding: 4, border: "1px solid #ddd", borderRadius: 4, fontSize: 11, background: getStatusColor(item.inventory_status || "작업지시완료"), color: item.inventory_status === "출고대기" ? "#000" : "#fff", fontWeight: 700 }}>
                        ...
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                          <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
                            <button onClick={() => handleMoveToContract(item, "order")} style={{ padding: "4px 6px", background: "#2e5b86", color: "#fff", border: "none", borderRadius: 4, fontSize: 10, cursor: "pointer", fontWeight: 600 }}>→수주</button>
                            <button onClick={() => handleMoveToContract(item, "branch")} style={{ padding: "4px 6px", background: "#6f42c1", color: "#fff", border: "none", borderRadius: 4, fontSize: 10, cursor: "pointer", fontWeight: 600 }}>→영업소</button>
                          </div>
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                        ...
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center", whiteSpace: "nowrap" }}>
                          <span style={{ fontWeight: 600, fontSize: 12 }}>{formatDateDisplay(item.contract_date)}</span>
                          <input type="date" value={item.contract_date || ""} onChange={(e) => updateField(item.quote_id, "contract_date", e.target.value)} style={{ width: 18, padding: 0, border: "none", background: "transparent", cursor: "pointer", opacity: 0.5 }} />
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                          <select value={normalizeSpec(item.spec) || item.spec || "3x6"} onChange={(e) => updateField(item.quote_id, "spec", e.target.value)} style={{ padding: 4, border: "1px solid #ddd", borderRadius: 4, fontSize: 12, fontWeight: 700 }}>
                            {SPEC_OPTIONS.map(spec => (<option key={spec} value={spec}>{spec}</option>))}
                          </select>
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee" }}>
                          <input key={item.quote_id + "_customer"} defaultValue={item.customer_name || ""} onBlur={(e) => updateField(item.quote_id, "customer_name", e.target.value)} style={{ width: 80, padding: 4, border: "1px solid #ddd", borderRadius: 4 }} placeholder="발주처" />
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                          <input key={item.quote_id + "_drawing"} defaultValue={item.drawing_no || ""} onBlur={(e) => { const val = e.target.value.replace(/\D/g, "").slice(0, 3); if (val && val !== item.drawing_no) updateField(item.quote_id, "drawing_no", val); }} style={{ width: 40, padding: 4, border: "1px solid #ddd", borderRadius: 4, textAlign: "center", fontWeight: 700, fontSize: 14 }} placeholder="-" />
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                          <select value={item.deposit_status || ""} onChange={(e) => updateField(item.quote_id, "deposit_status", e.target.value)} style={{ padding: 4, border: "1px solid #ddd", borderRadius: 4, fontSize: 11, background: item.deposit_status === "완료" ? "#28a745" : (item.deposit_status ? "#ffc107" : "#fff"), color: item.deposit_status === "완료" ? "#fff" : "#000", fontWeight: 600 }}>
                            <option value="">-</option>
                        ...
                        <td style={{ padding: 8, border: "1px solid #eee" }}>
                          <input key={item.quote_id + "_interior"} defaultValue={item.interior || ""} onBlur={(e) => updateField(item.quote_id, "interior", e.target.value)} style={{ width: 120, padding: 4, border: "1px solid #ddd", borderRadius: 4 }} placeholder="메모" />
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee" }}>
                          <input type="date" value={item.delivery_date || ""} onChange={(e) => updateField(item.quote_id, "delivery_date", e.target.value)} style={{ padding: 4, border: "1px solid #ddd", borderRadius: 4, fontSize: 11 }} />
                        </td>
                        <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                          <button onClick={() => handleDelete(item.quote_id, item.spec)} style={{ padding: "4px 8px", background: "#dc3545", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, cursor: "pointer" }}>삭제</button>
                        </td>
                      </tr>
                    );
                  })}
    ...
    {mainTab === "used" && (
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", overflow: "hidden" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: 40, color: "#888" }}>중고 재고가 없습니다.<br/>앱에서 등록해주세요.</div>
        ) : (
          <div style={{ overflowX: "auto" }}>
              ...
              <tbody>
                {usedItems.map((item) => (
                  <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                    ...
                    <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                      {item.photo_url ? (
                        <a href={item.photo_url} target="_blank" rel="noopener noreferrer">
                          <img src={item.photo_url} alt="사진" style={{ width: 50, height: 50, objectFit: "cover", borderRadius: 4 }} />
                        </a>
                    ...
                    <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                      <select value={item.status || "판매중"} onChange={async (e) => { await supabase.from("used_inventory").update({ status: e.target.value }).eq("id", item.id); loadInventory(); }} style={{ padding: 4, border: "1px solid #ddd", borderRadius: 4, fontSize: 11, background: item.status === "판매완료" ? "#6c757d" : "#28a745", color: "#fff", fontWeight: 600 }}>
                        <option value="판매중">판매중</option>
                        <option value="판매완료">판매완료</option>
                      </select>
                    </td>
                    <td style={{ padding: 8, border: "1px solid #eee", textAlign: "center" }}>
                      <button onClick={async () => { if (!confirm("삭제하시겠습니까?")) return; await supabase.from("used_inventory").delete().eq("id", item.id); loadInventory(); }} style={{ padding: "4px 8px", background: "#dc3545", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, cursor: "pointer" }}>삭제</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }} onClick={() => setShowAddModal(false)}>
        <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: "90%", maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <select value={newItem.inventory_status} onChange={(e) => setNewItem({ ...newItem, inventory_status: e.target.value })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8 }}>
              <option value="작업지시완료">작업지시완료</option>
              <option value="출고대기">출고대기</option>
              <option value="찜">찜</option>
              <option value="출고완료">출고완료</option>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <select value={newItem.container_type} onChange={(e) => setNewItem({ ...newItem, container_type: e.target.value })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8 }}>
              <option value="신품">신품</option>
              <option value="중고">중고</option>
              <option value="리스">리스</option>
            </select>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 600 }}>도면번호 <span style={{ color: "#2e5b86", fontWeight: 700, fontSize: 12 }}>(자동: {nextDrawingNo})</span></label>
              <input value={newItem.drawing_no || ""} onChange={(e) => setNewItem({ ...newItem, drawing_no: e.target.value })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8, boxSizing: "border-box" }} placeholder={String(nextDrawingNo)} />
            </div>
            <div style={{ width: 80 }}>
              <label style={{ display: "block", marginBottom: 4, fontWeight: 600 }}>수량</label>
              <input type="number" min={1} max={20} value={newItem.qty} onChange={(e) => setNewItem({ ...newItem, qty: Number(e.target.value) || 1 })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8, boxSizing: "border-box" }} />
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", marginBottom: 4, fontWeight: 600 }}>규격 *</label>
            <select value={newItem.spec} onChange={(e) => setNewItem({ ...newItem, spec: e.target.value })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8, fontWeight: 700 }}>
              {SPEC_OPTIONS.map(spec => (<option key={spec} value={spec}>{spec}</option>))}
            </select>
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", marginBottom: 4, fontWeight: 600 }}>입금</label>
            <select value={newItem.deposit_status} onChange={(e) => setNewItem({ ...newItem, deposit_status: e.target.value })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8 }}>
              <option value="">-</option>
              <option value="대기">대기</option>
              <option value="완료">완료</option>
              <option value="계약금">계약금</option>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input value={newItem.customer_name} onChange={(e) => setNewItem({ ...newItem, customer_name: e.target.value })} style={{ width: "100%", padding: 10, border: "1px solid #ddd", borderRadius: 8, boxSizing: "border-box" }} placeholder="발주처 입력" />
          </div>
